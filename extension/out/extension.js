...
axios_1.default.defaults.headers.common['User-Agent'] = 'MalPack-VSCode-Extension/1.0.0';
...
function getApiUrl() {
    ...
    let baseUrl = config.get('backendUrl', 'https://malpack-backend1.onrender.com');
    if (baseUrl.endsWith('/')) {
...
let findingsMap = new Map();
let currentPackage; // Tracks which package the user is currently navigating
let currentRuleFilter; // Tracks active rule filter for navigation
let currentScanDir = '';
...
function performCleanup(scanDir, pkgName) {
    if (scanDir && fs.existsSync(scanDir)) {
        try {
            fs.rmSync(scanDir, { recursive: true, force: true });
            if (pkgName) {
                findingsMap.delete(pkgName);
            }
...
function activate(context) {
    ...
    const sidebarProvider = new SidebarProvider_1.MalPackSidebarProvider(context.extensionUri);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(SidebarProvider_1.MalPackSidebarProvider.viewType, sidebarProvider));
    ...
    let disposable = vscode.commands.registerCommand('malpack.install', async () => {
        const webviewProvider = new webviewProvider_1.MalPackWebviewProvider(context);
        ...
        if (choice === 'PyPI Package') {
            ...
            await new Promise(resolve => {
                webviewProvider.showMethodSelectionPanel((m) => {
                    method = m;
                    resolve();
                });
            });
...
async function scanPackage(pkgName, method, apiUrl, diagnosticCollection, context) {
    if (vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0) {
        const rootPath = vscode.workspace.workspaceFolders[0].uri.fsPath;
    }
    ...
    return vscode.window.withProgress({
        ...
        try {
            ...
            let res;
            try {
                res = await axios_1.default.get(`https://pypi.org/pypi/${pkgName}/json`);
            }
            catch (e) {
            ...
            let target = releases.find((r) => r.url.endsWith('.whl'));
            if (!target)
                target = releases.find((r) => r.url.endsWith('.tar.gz') || r.url.endsWith('.zip'));
            if (!target)
                throw new Error(`No suitable archive found for ${pkgName} on PyPI.`);
            const archivePath = path.join(downloadDir, target.filename);
            const writer = fs.createWriteStream(archivePath);
            const downloadStream = await axios_1.default.get(target.url, { responseType: 'stream' });
...
async function scanSourceDirectory(sourceDir, pkgName, method, apiUrl, diagnosticCollection, context, cleanupDir) {
    const webviewProvider = new webviewProvider_1.MalPackWebviewProvider(context);
    return vscode.window.withProgress({
        location: vscode.ProgressLocation.Notification,
        ...
        try {
            ...
            if (method === 'llm') {
                ...
                const tasks = pyFiles.map((full, i) => limit(async () => {
                    ...
                    try {
                        ...
                        const res = await axios_1.default.post(`${apiUrl}/llm_file_check`, {
                            file_path: relPath,
                ...
                webviewProvider.showLlmVerdictPanel({
                    ...
                    alternatives: []
                }, async (install, actionStr, altName) => {
                    await processInstallDecision(pkgName, install, actionStr, altName, cleanupDir || '', context);
                }, async () => {
                    try {
                        const altRes = await axios_1.default.post(`${apiUrl}/suggest_alternatives`, { package_name: pkgName }, { timeout: 15000 });
                    ...
                    if (cleanupDir)
                        performCleanup(cleanupDir, pkgName);
            ...
            const ruleTasks = allFiles.map((filePath, i) => ruleLimit(async () => {
                ...
                try {
                    ...
                    const res = await axios_1.default.post(checkEndpoint, {
                    ...
                    if (res.data.status === "DANGER" || findings.length > 0) {
                        if (!findingsMap.has(pkgName)) {
                            findingsMap.set(pkgName, new Map());
                        }
                        ...
                        const diagnostics = findings.map(f => {
                            ...
                            const range = new vscode.Range(new vscode.Position(line, col), new vscode.Position(line, endCol));
            ...
            const summaryRes = await axios_1.default.post(`${apiUrl}/scan/summary`, { findings_data: allFileResults });
            ...
            webviewProvider.showVerdictPanel({
                ...
                webviewProvider.showHighLevelDetails(async (rId) => {
                ...
                }, () => navigateFinding('prev'), () => navigateFinding('next'));
            }, async (install, actionStr, altName) => {
                await processInstallDecision(pkgName, install, actionStr, altName, cleanupDir || '', context);
            }, async () => {
                try {
...
async function showCodeLevelDetails(pkgName, diagnosticCollection, ruleIdLimit) {
...
async function processInstallDecision(pkgName, install, actionStr, altName, scanDir, context) {
    if (!scanDir)
        return; // If local, we don't install anything.
    try {
        if (install) {
            let term = vscode.window.activeTerminal;
            if (!term)
                term = vscode.window.createTerminal("MalPack Install");
            term.show();
            if (actionStr === 'force') {
...
async function blockInstallation(pkgName, context) {
    ...
    if (activeVenv && fs.existsSync(activeVenv))
        venvs.add(activeVenv);
    try {
...
function showFeedbackPopup(pkgName) {
    vscode.window.showInformationMessage(`The analysis of ${pkgName} was correct?`, 'Yes', 'No').then(selection => {
        if (selection === 'Yes') {
...
function applyDecorations(editor) {
    ...
    const ranges = allFindings.map(f => {
        const range = new vscode.Range(new vscode.Position(f.line - 1, f.col_offset), new vscode.Position(f.line - 1, Math.max(f.end_col_offset, f.col_offset + 10)));
...
async function navigateFinding(direction) {
    ...
    let currentIndex = allFindingsList.findIndex(item => item.file === currentFile &&
        (item.f.line - 1 === currentPos.line));
    if (direction === 'next') {
        if (currentIndex === -1) {
            // Find first finding AFTER current cursor
            currentIndex = allFindingsList.findIndex(item => files.indexOf(item.file) > files.indexOf(currentFile) ||
        ...
        if (currentIndex === -1) {
            ...
            if (currentIndex === -1)
                currentIndex = allFindingsList.length - 1; // Wrap to end
        }
        else {
...
// ============================================================================
async function autoScanRequirementsTxt(document, context) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('# MalPack: SAFE') || line.includes('# MalPack: DANGER')) {
            continue;
        }
    ...
    const method = vscode.workspace.getConfiguration('malpack').get('defaultMethod') || 'rule_based';
    const apiUrl = getApiUrl();
...
async function performHeadlessScan(pkgName, method, apiUrl, context) {
    ...
    try {
        ...
        const extractDir = path.join(scanDir, 'source');
        fs.mkdirSync(extractDir, { recursive: true });
        const res = await axios_1.default.get(`https://pypi.org/pypi/${pkgName}/json`);
        const releases = res.data.urls;
        if (!releases || releases.length === 0) {
            throw new Error(`No downloadable files found for ${pkgName} on PyPI.`);
        }
        ...
        if (method === 'llm') {
            ...
            collect(extractDir);
            const llmRes = await axios_1.default.post(`${apiUrl}/llm_based_check`, { package_name: pkgName, files: pyFiles });
