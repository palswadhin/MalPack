...
exports.MalPackWebviewProvider = void 0;
const vscode = __importStar(require("vscode"));
class MalPackWebviewProvider {
    constructor(context) {
     ...
     */
    showMethodSelectionPanel(onMethodSelected) {
        this.panel = vscode.window.createWebviewPanel('malpackMethodSelect', 'MalPack: Select Analysis Method', vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        this.panel.webview.onDidReceiveMessage(message => {
            if (message.command === 'selectMethod') {
                onMethodSelected(message.method);
                this.panel?.dispose();
            }
     ...
     */
    showVerdictPanel(result, onNext, onInstall, onSuggest, onDispose) {
        this.scanResult = result;
        this.onInstallCallback = onInstall;
        if (!this.panel) {
            this.panel = vscode.window.createWebviewPanel('malpackVerdict', `MalPack Scan: ${result.packageName}`, vscode.ViewColumn.One, {
                enableScripts: true,
                retainContextWhenHidden: true
            });
        }
     ...
     */
    showLlmVerdictPanel(result, onInstall, onSuggest, onDispose) {
        this.scanResult = result;
        this.onInstallCallback = onInstall;
        this.panel = vscode.window.createWebviewPanel('malpackLlmVerdict', `MalPack LLM Scan: ${result.packageName}`, vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
     ...
     */
    showHighLevelDetails(onShowCode, onBack, onSuggest, onPrev, onNext) {
        if (!this.scanResult || !this.panel)
            return;
        ...
        this.panel.webview.onDidReceiveMessage(message => {
            switch (message.command) {
                ...
                case 'back':
                    if (this.scanResult) {
                        this.showVerdictPanel(this.scanResult, (rId) => this.showHighLevelDetails(onShowCode, onBack, onSuggest, onPrev, onNext), this.onInstallCallback, onSuggest);
                    }
                    break;
                case 'install':
                    this.panel?.dispose();
    ...
    showClassifierComingSoon() {
        this.panel = vscode.window.createWebviewPanel('malpackClassifier', 'MalPack: Classifier Analysis', vscode.ViewColumn.One, { enableScripts: true });
    ...
    updateAlternatives(alternatives, error) {
        if (this.panel) {
...
<body>
    ...
    getHighLevelDetailsHtml(result) {
...
exports.MalPackWebviewProvider = MalPackWebviewProvider;
